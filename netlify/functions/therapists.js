    ...
    switch (httpMethod) {
      case 'GET':
        if (path.includes('/institute/')) {
          ...
          const result = await query('SELECT * FROM therapists WHERE institute_id = $1 ORDER BY name', [instituteId]);
      ...
      case 'POST':
        const { institute_id, name, experience, certification, additional_certification, bio, image_url } = JSON.parse(event.body);
        const insertResult = await query(
          `INSERT INTO therapists (institute_id, name, experience, certification, additional_certification, bio, image_url) 
           VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
          [institute_id, name, experience, certification, additional_certification, bio, image_url]
        );
      ...
      case 'DELETE':
        ...
        const deleteResult = await query('DELETE FROM therapists WHERE id = $1', [pathParameters.id]);
